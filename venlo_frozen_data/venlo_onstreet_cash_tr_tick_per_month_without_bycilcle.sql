delete from public.onstreet_cash_transport_vs_tickets_per_month
...
with m_range AS (SELECT generate_series('1jan2019', now()::date - INTERVAL '1' MONTH, '1 month'::interval) as start_of_month),
     ...
     unique_meter_codes as (
         ...
         WHERE client_id = ${account_id}
         union
         SELECT 49, '008000'  -- fiets apparaat erbij
     ),
     ...
     still_present_in_apparaat_start_of_month as (
         ...
         --   AND b.payment_type_id = 1   -- xx
             ...
             AND b.client_id = ${account_id}  -- keep this, with this an index is used and the query
         GROUP BY 1, 2, 3, 4, 5),
...
insert into public.onstreet_cash_transport_vs_tickets_per_month
select a.client_id,
       a.meter_code,
